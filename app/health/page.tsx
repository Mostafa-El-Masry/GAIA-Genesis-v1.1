      ...
      <section style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        <div className="summary-cards">
          <div className="card">
            ...
            <div style={{ fontSize: "1.8rem", fontWeight: 700 }}>{Math.round(m.avg) || "—"} <span style={{ fontSize: "0.9rem" }}>mg/dL</span></div>
          </div>
          <div className="card">
            <div style={{ opacity: 0.8 }}>Estimated A1C</div>
            <div style={{ fontSize: "1.8rem", fontWeight: 700 }}>{m.a1c ? m.a1c.toFixed(1) : "—"} <span style={{ fontSize: "0.9rem" }}>%</span></div>
          </div>
          <div className="card">
            <div style={{ opacity: 0.8 }}>Time In Range</div>
            <div style={{ fontSize: "1.8rem", fontWeight: 700 }}>{m.tir ? m.tir.toFixed(0) : 0}%</div>
          </div>
          <div className="card">
            <div style={{ opacity: 0.8 }}>High / Low (30d)</div>
            <div style={{ fontSize: "1.2rem", fontWeight: 700 }}>{m.highs} high · {m.lows} low</div>
          </div>
        </div>
